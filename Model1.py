...
from KnowledgeTracing.DirectedGCN.GAT import GAT
...
class DKT(nn.Module):
    ...
    def forward(self, x):
        ...
        '''choose 50'''
        loaded_array = np.load('../../Dataset/node2vec/emb/ASSIST17_kg_pk17_128_80_10_5_20_1.00_1.00.embQ.npy')
        tensor_array = torch.tensor(loaded_array)
        tensor_array = tensor_array.to(device)
        tensor_array_expanded = tensor_array.repeat(2, 1)
        ...
        ques_h = alpha * tensor_array_expanded + (1 - alpha) * tensor_array_expanded1
        ques_h = ques_h.float()
        # ques_h=torch.cat([tensor_array_expanded, tensor_array_expanded1], -1)
        ''''
        question_embed_method1 = tensor_array_expanded
        question_embed_method2 = tensor_array_expanded1

        # Compute attention scores between the two sets of question node embeddings
        att_scores = torch.matmul(question_embed_method1, question_embed_method2.t())  # Compute attention scores

        # Compute attention weights using softmax
        att_weights_method1 = F.softmax(att_scores, dim=1)
        att_weights_method2 = F.softmax(att_scores, dim=0)

        # Fuse the two types of embeddings using the attention weights
        ques_h = torch.matmul(att_weights_method1, question_embed_method2) + torch.matmul(
             att_weights_method2,question_embed_method1,)
        ...
        # self.adj_out = update_adjacency_matrix(self.adj_out, out_d, device).float()
